...
// Чтение данных mailslot (блокирующий вызов)
   ret:=ReadFile(Mailslot, buffer, 256, NumberOfBytesRead,Nil);                  // хз почему 256 выше определен мах размер сообщения 0 т.е. без ограничений. если передать >256 то все развалится
   If ret then
    begin
