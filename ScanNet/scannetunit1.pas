   ...
   j:=i;
     ...
     If s=INVALID_SOCKET then ShowMessageFmt('Ошибка %d при создании сокета', [WSAGetLastError])
         ...
         begin
              ...
              If ret=SOCKET_ERROR then Form1.Memo1.Lines.Add('Порт '+IntToStr(j)+' не отвечает')
              else
...
begin
   ...
   If WSAStartup(MAKEWORD(2,2), wsd)<>0 then ShowMessageFmt('Ошибка %d при инициализации интерфей-са сокетов', [WSAGetLastError])
   else
       begin
           ii:=1;
