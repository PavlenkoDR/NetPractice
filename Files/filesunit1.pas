...
begin
      UNCFileName:=Edit1.Text;
       ...
       If FileHandle=INVALID_HANDLE_VALUE then ShowMessage(SysErrorMessage(GetLastError))
       else
        ZeroMemory(@buffer, sizeof(buffer)) ;
        if(ReadFile(FileHandle, buffer, sizeOf(Buffer), BytesRead, Nil) = False) then ShowMessage(SysErrorMessage(GetLastError))
           else
           begin
            ReadString:=StrPas(@buffer);
            ShowMessage(readString);
            Memo1.Lines.Text:=ReadString;
                 if (CloseHandle(FileHandle) = False) then ShowMessage(SysErrorMessage(GetLastError))
           end;
      ...
      begin
       ...
       if (WriteFile(FileHandle, buffer, BytesToWrite, BytesWritten, Nil)) then
         begin
          ...
          if not (CloseHandle(FileHandle))then             ShowMessage(SysErrorMessage(GetLastError));
         end
...
begin
 ...
 If MessageDlg('Вы действительно хотите удалить файл ?',mtWarning, mbYesNo, 0)=mrYes Then
   begin
    if DeleteFile(PChar(Edit1.Text)) then ShowMessage('Файл удален!')
    else ShowMessage(SysErrorMessage(GetLastError));
   end;
end;
